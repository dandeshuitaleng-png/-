...
| 核心体验 | 直接抽取、回答两题、收藏 | 一键推荐、两题问答、预算/距离/忌口/出餐速度、推荐理由、收藏 |
| 参赛目标 | 功能验证 | 初赛说明文档 + iOS 高保真原型 + SwiftUI MVP 骨架 |

## 文件说明

- `初赛作品说明.md`：按官方初赛作品说明模板补齐的参赛文案。
- `iOS原型/index.html`：可直接在浏览器打开的 iPhone 风格高保真原型，用于截图、演示和打磨交互。
- `swiftui/EatWhat2Prototype.swift`：SwiftUI MVP 骨架，内置 30 条校园周边样本餐饮数据，方便后续在 Xcode 中创建 iOS 工程时迁移。
